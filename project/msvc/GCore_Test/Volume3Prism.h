#ifndef _VOLUME_THREE_PRISM_H_
#define _VOLUME_THREE_PRISM_H_
...
private:
	// 判断两个双精度浮点是否相等
	static bool doubleEquals(double v1, double v2)
	{
	...
	// 获取三个double的最大值
	static double maxOf(double x1, double x2, double x3)
	{
		return MAX_OF(MAX_OF(x1, x2), x3);
	}

	// 获取三个double的最小值
	static double minOf(double x1, double x2, double x3)
	{
		return MIN_OF(MIN_OF(x1, x2), x3);
	}

	// 求两个点的距离
	static double getDistanceOfTwoPoints(
	...
	// 求一个三角形的面积
	static double getAreaOfTriangle(
		double xa, double ya, double za,
		double xb, double yb, double zb,
		double xc, double yc, double zc)
	{
	...
	// 求一个不规则三棱柱的体积
	// 要求up面在上，down面在下
	static double getVolumeOfThreePrism(
	...
	static double getVolumeOfFivePoints(
		double xa, double ya, double za,
		double xb, double yb, double zb,
		double xc, double yc, double zc,
	...
	// 求一个一般三棱锥的体积
	static double getVolumeOfThreePyramid(
		double xa, double ya, double za,
		double xb, double yb, double zb,
		double xc, double yc, double zc,
		double xd, double yd, double zd)
