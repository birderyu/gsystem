...
cs_bool CsUdpSocket::Open(cs_bool nResue)
{
	CS_ASSERT(m_pSocket_Ex);
	return ((CsUdpSocket_Ex*)m_pSocket_Ex)->Open(nResue);
}

cs_bool CsUdpSocket::Open(const CsSockAddress &tSockAddr, cs_bool nResue)
{
	CS_ASSERT(m_pSocket_Ex);
	return ((CsUdpSocket_Ex*)m_pSocket_Ex)->Open(tSockAddr.m_pSockAddress_Ex, nResue);
}

cs_void CsUdpSocket::Close()
{
	CS_ASSERT(m_pSocket_Ex);
	((CsUdpSocket_Ex*)m_pSocket_Ex)->Close();
}

cs_bool CsUdpSocket::SendTo(const cs_char *pBufData, cs_int nLen, const CsSockAddress &tPeer)
{
	CS_ASSERT(m_pSocket_Ex);
