...
template<typename T>
struct GDListNode
	: public GPreviousNextNodeT<GDListNode<T>>
	, public GValueNodeT<T>
	, public GNewT<GDListNode<T>>
{
	/****************************************************************************
	**
	** @name GDListNode
	...
	** @param [in] previous {GDListNode *} 上一个节点的指针
	** @param [in] next {GDListNode *} 下一个节点的指针
	**
	****************************************************************************/
	GDListNode(const T &value = T(),
		GDListNode *previous = GNULL,
		GDListNode *next = GNULL);
...
**
********************************************************************************/
template<typename T, typename NodeT = GDListNode<T>>
class GDList 
...
public:
	...
	** @param [in] list {const GDList &} 双向链表
	**
	****************************************************************************/
	GDList(const GDList &list);

	/****************************************************************************
	**
	** @name GDList
	** @brief 移动构造函数（move constructor）
	** @param [in] list {GDList &&} 双向链表
	**
	****************************************************************************/
	GDList(GDList &&list);
	...
	** @brief 拷贝运算符
	** @param [in] list {const GDList &} 双向链表
	** @return {GDList &} 双向链表的引用
	**
	****************************************************************************/
	GDList &operator=(const GDList &list);
