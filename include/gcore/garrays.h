...
public:
	...
	** @return {T *} 返回创建数组的首地址，若创建失败，则返回GNULL
	**
	** 创建一个元素个数为size的数组，会调用每一个元素的默认构造函数。
	**
	****************************************************************************/
	template <typename T> 
	static T *CreateArray(gsize size) GEXCEPT(false);
	...
	****************************************************************************/
	template <typename T> 
	static T *CreateArray(gsize size, const T &copyable) GEXCEPT(false);

	/****************************************************************************
	**
	** @name CreateArray
	** @brief 静态方法，拷贝创建数组的一部分元素成为一个新的数组
	** @template T 数组元素的元素个数
	** @param [in] copy_arr {const T *} 被拷贝的数组的首地址
	...
	****************************************************************************/
	template <typename T> 
	static T *CreateArray(const T *copy_arr, gsize copy_start_pos, 
	...
	****************************************************************************/
	template <typename T> 
	static T *CreateArray(gsize size, gsize start, const T *copy_arr, 
	...
	****************************************************************************/
	template <typename T> 
	static T *ResizeArray(T *old_arr, gsize old_size, gsize new_size) 
		GEXCEPT(false);
	...
	****************************************************************************/
	template <typename T> 
	static T *ResizeArray(T *old_arr, gsize old_size, gsize new_size, 
		const T &copyable) GEXCEPT(false);
	...
	****************************************************************************/
	template <typename T> 
	static T *ResizeArray(T *old_arr, gsize old_start, gsize old_size, 
	...
	****************************************************************************/
	template <typename T> 
	static T *ResizeArray(T *old_arr, gsize old_start, gsize old_size, 
		gsize new_size, gsize new_start, const T &copyable) GEXCEPT(false);
	...
	** @template T 数组元素的类型
	** @param [in] arr {T *} 数组的首地址
	** @param [in] size {gsize} 数组的长度
	** @param [in] pos {gsize} 待移除元素的位置，从0开始计数
	** @return {T *} 移除元素后的数组的首地址，若移除失败，则返回GNULL
	**
	** 对移除掉的元素调用析构函数
	**
	****************************************************************************/
	template <typename T> 
	static T *RemoveArrayElementAt(T *arr, gsize size, gsize pos) 
		GEXCEPT(false);
	...
	****************************************************************************/
	template <typename T> 
	static gvoid DestoryArray(T *arr, gsize size) GNOEXCEPT;
	...
	****************************************************************************/
	template <typename T> 
	static gvoid ClearArray(T *arr, gsize size) GNOEXCEPT;
	...
	** @param [in] arr {T *} 数组的首地址
	** @param [in] size {gsize} 数组的长度
	** @param [in] copy_arr {const T *} 被拷贝的数组，需至少包含size个元素
	**
	** 将数组copy_arr的元素拷贝到arr中，arr的元素个数不会改变
	**
	****************************************************************************/
	template <typename T> 
	static gvoid CopyArrayFrom(T *arr, gsize size, const T *copy_arr) 
		GEXCEPT(false);
	...
	** @param [in] arr {T *} 数组的首地址
	** @param [in] size {gsize} 数组的长度
	** @param [in] copy_arr {const T *} 被拷贝的数组
	** @param [in] copy_size {gsize} 被拷贝的数组的长度
	** @return {T *} 拷贝元素之后的数组首地址，若拷贝失败，则返回GNULL
	**
	...
	****************************************************************************/
	template <typename T> 
	static T *CopyArrayFrom(T *arr, gsize size, const T *copy_arr, 
		gsize copy_size) GEXCEPT(false);
};
