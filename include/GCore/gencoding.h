...
public:
	...
	guint8 GetUnitSizeOfUtf8Character(const gchar8 *utf8);

	// 将一个UTF-8字符转为对应的Unicode编码
	guint32 Utf8CharacterToUnicode(const gchar8 *utf8);

	// 将一个Unicode编码转换为对应的UTF-8字符，返回字符占据的元素单位的个数
	guint8 UnicodeToUtf8Character(guint32 unicode, gchar8 *utf8);

	// 将一个Unicode编码转换为对应的UTF-16字符，返回字符占据的元素单位的个数
	guint8 UnicodeToUtf16Character(guint32 unicode, gchar16 *utf16);

	// 将一个Unicode编码转换为对应的UTF-32字符，返回字符占据的元素单位的个数
	guint8 UnicodeToUtf32Character(guint32 unicode, gchar32 *utf32);
