...
GRefCounter<ClassT>::GRefCounter(ClassT *ptr, gsize count)
...
template <typename ClassT> GINLINE
GRefCounter<ClassT>::~GRefCounter()
{
	delete m_pPointer;
}

template <typename ClassT> GINLINE
gsize GRefCounter<ClassT>::Add()
{
	return GAtom::IncrementAndFetch(&m_nCount);
}

template <typename ClassT> GINLINE
gvoid GRefCounter<ClassT>::Release()
{
...
template <typename ClassT> GINLINE
gsize GRefCounter<ClassT>::Count() const
{
	return GAtom::GetValue(&m_nCount);
}

template <typename ClassT> GINLINE
ClassT * GRefCounter<ClassT>::Pointer()
{
	return m_pPointer;
}

template <typename ClassT> GINLINE
const ClassT *GRefCounter<ClassT>::Pointer() const
{
	return m_pPointer;
}
