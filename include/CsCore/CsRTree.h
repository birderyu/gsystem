...
template<typename DATATYPE, typename ELEMTYPE, 
	cs_size_t NUMDIMS, typename ELEMTYPEREAL = ELEMTYPE, 
	cs_size_t TMAXNODES = 8, cs_size_t TMINNODES = TMAXNODES / 2>
class CsRTree
{
protected:
...
public:
	...
	cs_void Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId);

	/// Remove entry
	/// \param a_min Min of bounding rect
	/// \param a_max Max of bounding rect
	/// \param a_dataId Positive Id of data.  Maybe zero, but negative numbers not allowed.
	cs_void Remove(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId);
	...
	private:
		...
		friend CsRTree; // Allow hiding of non-public functions while allowing manipulation by logical owner
	};
...
protected:
	...
	void Reset();
	void CountRec(Node* a_node, cs_size_t& a_count);
